...
export function Owl() {
  ...
  const fbx = useFBX('/assets/models/Owl_Animations.fbx');
  const texture = useTexture('/assets/models/T_Owl.png');
  ...
  useEffect(() => {
    ...
    // Öncelik: Attack
    const hitAnimName = names.find(n => n.toLowerCase().includes('attack')) || names.find(n => n.toLowerCase().includes('hit'));
    ...
    if (isHit && hitAnimName) {
      ...
      // Normalde IDLE_A oyna (Yavaş: 0.4)
...
useFBX.preload('/assets/models/Owl_Animations.fbx');
useTexture.preload('/assets/models/T_Owl.png');
